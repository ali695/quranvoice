...
export async function getAyahAudioFile(
  reciterId: string,
  verseKey: string,
): Promise<{ url: string; verseKey: string; segments?: Array<[number, number, number, number]> } | null> {
  if (!isFoundationConfigured() || !/^\d+$/.test(reciterId)) return null;
  const numericId = Number(reciterId);
  const json = await foundationFetch<FoundationAyahAudio>(
  ...
  if (!file) return null;
  return { url: file.url, verseKey: file.verse_key, segments: file.segments };
}
